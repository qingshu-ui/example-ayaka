...
import io.github.qingshu.ayaka.example.utils.SendUtils
...
class Roulette : BotPlugin {
    ...
    private fun start(groupId: Long, userId: Long, matcher: Matcher, bot: Bot) {
        ...
        expiringMap.put(
            ...
            EAConfig.plugins.roulette.timeout.toLong(),
            TimeUnit.SECONDS,
        )
        SendUtils.at(
            userId, groupId, bot,
    ...
    private fun updateShotData(
        ...
        data.progress++
        data.hitCount++
        return defaultQuotations[8].replace("$1", EAConfig.base.nickName)
    }
